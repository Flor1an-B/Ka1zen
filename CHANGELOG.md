...
**Both inference engines updated again: mlx-vlm 0.6.15 → 0.7.0 and llama.cpp b10509 → b10894.** As with every engine bump, every claim below was validated end-to-end in an isolated environment that never touches the shipping runtime, on the M5 Max, before the pin changed. This cycle also carries a known, deliberately accepted tradeoff — see below.

### Changed

- **Pinned mlx-vlm 0.6.15 → 0.7.0** (0.6.16 and 0.6.17 skipped over — 0.6.16 is now denylisted). This is the resolution of a regression opened two weeks ago: 0.6.16 introduced a reproducible ~20% Qwen 3.6 35B-A3B MoE + MTP decode-speed regression (~115 → ~90 t/s) with correctness unaffected. Rather than bump into that regression, Ka1zen stayed on 0.6.15 pending an upstream fix. 0.7.0 includes several targeted fixes (#2120 native-MTP-drafter detection, #2125 "verify qwen3_5_moe MTP", #2133 "Require MLX 0.32.2 for faster speculative decoding", #2140 Qwen verifier QMV optimization, #2152 hybrid-model cache fix) that **reduce but do not eliminate** the regression: a same-session A/B measured 0.7.0 at ~15% slower than 0.6.15 (~115.4 → ~98.0 t/s), versus 0.6.16's ~20%. Root cause remains unconfirmed upstream. This was a deliberate, discussed tradeoff to pick up 0.7.0's other fixes rather than hold indefinitely — see `DependencyManifest.swift` for the full record. Everything else non-regressed: Gemma 4 E4B dense + MTP clean (102.1 t/s), Gemma 4 26B-A4B MoE + MTP clean (82.2 t/s), DiffusionGemma mxfp4 clean, tool-calls clean (including new multi-tool-call and quoted-argument support), a 2-turn conversation clean. Transitively bumps **mlx 0.32.0 → 0.32.2** (0.7.0's new floor) and pulls in `mlx-audio >= 0.4.8` as a new, unused transitive dependency. transformers floor unchanged (>= 5.14.0).
- **Pinned llama.cpp bumped b10509 → b10894** (385 commits). Clean across the board: Qwen 3.6 27B GGUF DFlash (35.1 t/s, 61.5% accept), Qwen 3.6 35B-A3B MoE self-spec MTP (107.1 t/s, 87.3% accept), Gemma 4 26B-A4B MoE MTP via the separate `*-assistant` drafter (109.0 t/s, 81.4% accept, no corruption), a tool-call and a 2-turn conversation both clean. Worth noting: **#28183 "fix gemma4-assistant"** fixed a real bug in loading a drafter with only MTP layers from block 0 — exactly Ka1zen's Gemma `*-assistant` pairing shape — and the Gemma MTP test above ran on the fixed side of that change. Also ships Metal 4.0 tensor API support for M5+/A19+ (#27461) and continued Metal `fa-vec` tunings for M5/M5 Pro. **Gemma 4 31B GGUF DFlash was NOT re-verified this round** — the plain dense GGUF target model is still absent from the local model cache (same gap as the last two cycles, unrelated to this bump).
...
- **Pinned mlx-vlm 0.6.7 → 0.6.10** (0.6.8 and 0.6.9 folded in). Non-regression validated in an isolated venv against real installed weights before the bump: the long-context MoE + MTP reproducer (Blaizzy/mlx-vlm#1317) stays clean (111.6 t/s, coherent French, zero CJK leakage); a 2-turn conversation with Fast Mode on stays coherent with no cross-turn confusion; Gemma 4 E4B dense + MTP clean (110 t/s); DiffusionGemma mxfp4 still loads and generates with no "Biases must be provided". This bump also carries two real upstream fixes that land squarely on Ka1zen's setup: **#1748**, "prompt-cache prefix reuse corrupting rotating KV caches" — a raw array slice that was only valid for a flat cache silently mis-trimmed a sliding-window cache (or crashed outright once speculative decoding wraps it in `BufferedRotatingKVCache`, exactly what MTP Fast Mode does); and **#1754**, "isolate Qwen MTP shards during sanitization" — an attached `language_model.mtp.*` draft shard could leak its norm offset into the base model's own RMSNorm values, corrupting output even with MTP disabled. Server `timings` also gained `draft_kind`/`draft_rounds`/`draft_n`/`draft_n_accepted` fields (#1788) — not yet surfaced in the Ka1zen UI. Dependencies mlx 0.32.0 / mlx-lm 0.31.3 / transformers 5.14.1 unchanged.
- **Pinned llama.cpp bumped b10107 → b10273** (168 commits since b10107, in two hops via the previously-tested b10158). Non-regression proven before shipping on Qwen 3.6 27B dense, Qwen 3.6 35B-A3B MoE, and Gemma 4 26B-A4B MoE, all with MTP Fast Mode: coherent output, speeds within measurement noise. Also specifically re-tested **GGUF DFlash** (Qwen 3.6 27B, 28.2 t/s, coherent) because this range both introduces (#26531) and fixes (#26577) a DFlash tensor-reshape bug — Ka1zen's jump skips the broken intermediate state, confirmed clean at b10273 regardless — and plain (non-Fast-Mode) loading of an MTP-embedded GGUF, because #26296 changes llama.cpp to skip loading MTP tensors when `--spec-type draft-mtp` isn't passed (14.6 t/s baseline, coherent, memory-optimization only). One PR in this range claimed a ~10% MTP speedup on Qwen 3.6 35B-A3B (#26177, already applied at b10158) that turned out to be a no-op for Ka1zen — it only fixes llama.cpp's automatic `--fit` layer-offload count, and Ka1zen always passes an explicit `-ngl 999`, which `--fit` never touches. No `--spec-type`/`--model-draft` flag changes anywhere in the range.
...
- **DiffusionGemma now loads and generates.** Google's `diffusiongemma-26B-A4B-it` (block-diffusion LM) is advertised since 0.4.0, but on the previously pinned mlx-vlm 0.6.3 the mxfp4 build crashed at load — `Biases must be provided` — so it never actually ran on the shipped runtime. mlx-vlm 0.6.5 fixes the microscaling dequant path; verified through `mlx_vlm.server` (Ka1zen's exact path): loads clean, generates coherent prose, no leaked structural tokens, `finish_reason: stop`. The existing per-token-`logprobs` suppression for diffusion models stays (they have none by construction).
- **mlx-vlm 0.6.4 denylisted — it broke Qwen 3.6.** 0.6.4 re-ran weight sanitization on already-converted MLX safetensors, corrupting the logits of every `qwen3_5` model: Qwen 3.6 dense 27B **and** MoE 35B-A3B produced random-unicode garbage on any prompt, trivial ones included (upstream Blaizzy/mlx-vlm#1521; Gemma was unaffected). Anyone who manually updated to 0.6.4 got garbage output. Ka1zen now flags 0.6.4 as known-bad in **Settings → Runtime Health**, and `install.sh` / README pin 0.6.5, which fixes it (idempotent sanitize, upstream #1550/#1541/#1540).

### Changed

- **Pinned mlx-vlm 0.6.3 → 0.6.5** (transitively pulls **mlx 0.31.2 → 0.32.0** and **transformers 5.9.0 → 5.14.1**; mlx-lm 0.31.3, huggingface-hub 1.17.0, hf-transfer 0.1.9 unchanged). Non-regression validated in an isolated venv before the bump: Qwen 3.6 dense (27B mxfp8) and MoE (35B-A3B 8bit), DiffusionGemma mxfp4, Gemma 4 26B-A4B, and Gemma 4 31B dense with MTP Fast Mode — all clean, no corruption, no CJK leakage on long generation. The in-app **Runtime Health** validated set and `install.sh` pins were updated to match. Note: `mflux` (the image-generation engine) declares `mlx<0.32.0`, so a fresh `pip install` prints a dependency-conflict warning — it is cosmetic; image generation is verified working on mlx 0.32.0 (the cap is conservative, not a real incompatibility).
- **Pinned llama.cpp bumped b9859 → b10054.** The GGUF backend (`llama-server`) gains streaming and Fast Mode reliability from the upstream delta: an SSE keepalive so a slow prefill never drops a healthy connection, a `load_models()` deadlock fix, a prompt-cache RAM limit, and several draft-model fixes (fit-vs-load, stale tensor-split params, duplicate draft downloads, a `draft-simple` crash, a DFlash tokenizer-mismatch on conversion). It brings **no decode speedup** — the delta has no Apple-Silicon kernel work — so this is a pure reliability bump. Non-regression proven before shipping: on the same Qwen 3.6 27B model at temp 0, **baseline, MTP, and DFlash output are byte-identical between b9859 and b10054** (and DFlash/MTP remain byte-identical to baseline, i.e. still lossless), with generation speed within measurement noise. The managed binary shipped with the app is updated (previous build backed up); `install.sh` pins b10054.
- **Faster Qwen 3.6 prompt processing.** mlx-vlm 0.6.5 ships a large Qwen3.5-family prefill acceleration, which lowers time-to-first-token on long inputs — most visible on web-search turns that inject thousands of tokens of results (an ~8K-token web-search prompt reached first token in ~2 s on the M5 Max).
- **New models available through the updated engine.** 0.6.4/0.6.5 add architecture support for, among others, Ornith 1.0 35B (`qwen3_5_moe` split experts), MiniMax M3, and DeepSeek V4 — these now load via the MLX backend when you download them.

## [0.5.0] — 2026-07-03

Two headline items: a **web-search reliability overhaul** (Ka1zen stopped telling users that real, recently-released products "don't exist"), and **DFlash** speculative decoding for code on the GGUF backend. Plus the pinned llama.cpp bump to b9859 that makes DFlash possible.

### Added

- **Fast Mode: optional DFlash speculative decoding for code, on the GGUF backend.** DFlash (DeepSeek DeepSpec) is a block-diffusion drafter that llama.cpp now runs natively on Metal (`--spec-type draft-dflash`). A new "DFlash for code" sub-toggle appears under Settings → Performance when Fast Mode is on (default off). It roughly doubles code generation on the slow dense targets, lossless, measured on the M5 Max at b9859: Qwen 3.6 27B 15.7 → 41.3 tok/s (x2.63) and Gemma 4 31B 26.9 → 51.5 tok/s (x1.91). It is a deliberate opt-in rather than the default because DFlash only pays off on code: on chat and web-search prose its draft acceptance collapses (measured ~42-48% vs ~86% on code) and it is neutral to slightly slower than plain MTP, and the server cannot tell per-request whether a turn is code or chat. So MTP stays the everyday default; turn DFlash on for coding sessions. Enabling the toggle downloads the drafter (a separate `*-DFlash-GGUF` repo, unlike the bundled MTP head) with a notification when it is ready. Safeguards: the drafter's GGUF architecture is verified to be `dflash` before use; weight-variant targets (QAT, abliterated, uncensored fine-tunes) and Mixture-of-Experts targets are excluded and fall back to MTP, because a drafter trained on the standard weights gives near-zero acceptance on retrained weights and would make generation slower, not faster.

### Changed

- **Web search reliability overhaul.** A run of fixes turned web search from unreliable into dependable, after it was observed confidently telling users that real, recently-released products (a phone launched after the model's training cutoff) were a "scam" or "did not exist", and flip-flopping across turns. The changes, end to end:
  - **It fires when it should.** The code-generation detector, which suppresses auto-search so "write me an HTML page" is not derailed, used to trip on any message merely containing coding vocabulary — "Compare the Qwen-Coder-Next and Qwen 3.6 models" was misread as a code request and never searched. It now requires the creation verb to lead the request, so questions search normally while imperatives like "Écris une fonction" stay suppressed.
  - **It searches a real query, not your sentence.** The raw conversational message ("mon fils a un POCO X6 Pro, que me conseilles-tu…") was sent verbatim to the engine, which read it as prose and returned irrelevant pages. A quick reformulation step now rewrites it into a focused query — key entities and model names kept verbatim, filler dropped, current year added for recent topics — with a fall back to the raw text if it fails. This is the biggest lever for relevant results.
  - **The model can't fake a search or dismiss real results.** It used to write an imaginary search and imaginary results inside its reasoning ("Search Query… / Result: zero official results / Confirmed: fake") and answer from that hallucination; it now must emit a real tool call and use only the results returned. It no longer defaults to "doesn't exist / it's a scam" for anything it hasn't heard of (absence of confirmation is not evidence of nonexistence, and the user never has to prove a real thing exists), and it no longer labels live results "simulated" or "hypothetical" just because an event postdates its training.
  - **A blocked page falls back to search.** When a pasted URL can't be fetched (big sites such as mi.com and Amazon block automated fetching with HTTP 403), Ka1zen used to feed the model the error as if it were page content and skip search entirely, leaving it to confabulate. It now falls back to a web search on the subject (derived from the URL slug) and is honest that the fetch failed — a blocked page is not evidence the product is fake.
  - **Vision models participate too.** The tools and the full search rules were withheld from the vision (mlx_vlm) route on an old assumption it could not do tool calling; mlx_vlm now implements OpenAI-style tool calling (verified on Gemma 4 and Qwen 3.6), so vision models get the same tools and rules as text models. A forced background search on the reformulated query remains the reliable grounding floor, since local models often answer from memory rather than searching on their own.
- **Pinned llama.cpp bumped b9585 → b9859.** Brings native DFlash speculative decoding (above), the `dflash` / `eagle3-for-qwen3.6` spec-decoding work, a `/v1/models` dedup fix, and assorted server hardening. Non-regression verified before the bump: on the same model and config, baseline decode is identical and MTP draft acceptance is bit-identical between b9585 and b9859 (b9859 is marginally faster). The managed binary shipped with the app is updated; a Homebrew fallback install now also pins b9859.
- **First-run wizard no longer pre-enables Fast Mode.** The "Your first model" wizard used to check "Enable Fast Mode" by default and describe it as "byte-identical output", which contradicted the Settings design (Fast Mode is opt-in, off by default, behind an experimental-warning dialog) and could hand a brand-new user experimental speculative decoding on their first web-search or long-context chat. The wizard checkbox now defaults off and is labelled honestly as experimental. A new user gets a clean baseline first and can enable Fast Mode later from Settings once they understand the trade-off.
- **Response bubble fills the window width with a constant margin.** The assistant bubble was capped at a fixed 1100 pt, leaving a large empty gutter on wide windows. It now expands to the available width, keeping a constant 24 pt margin from the window edge that holds as the window is resized.
- **Default system prompt avoids the em-dash.** Ka1zen now instructs models to use commas, parentheses, colons, or periods instead of em-dash or en-dash characters, for output that reads less machine-generated.
...
Every performance and correctness claim below was **validated empirically, not assumed**: isolated-venv regressions that never touch the shipping runtime, draft-loading confirmed in the server console (not inferred), and a deterministic reproducer — a French web-search prompt that used to degenerate into a Chinese loop — verified clean across temperatures and both model families before any guard was lifted. **Existing MLX users are unaffected** — GGUF is additive and only engages when you download a GGUF model; if llama.cpp isn't installed, GGUF degrades gracefully with an actionable prompt while MLX keeps working.

### Added

- **GGUF inference backend (llama.cpp / `llama-server`).** A third launch path in `ModelLauncher` (after `mlx_lm.server` / `mlx_vlm.server`), Metal-optimized (`-ngl 999 --flash-attn on --jinja`), serving the same OpenAI API on a dedicated port (8082) so the rest of the app is unchanged. The Model Manager and picker split installed models into **MLX** and **GGUF · llama.cpp** sections.
- **Vision for GGUF (image input).** GGUF repos that ship an `mmproj` projector now load it (`--mmproj`) so the model can see images — verified on Qwen 3.6 27B/35B-A3B, Qwen 3.5 4B, and Gemma 4 26B/31B (and Gemma 4 12B "unified" on llama.cpp ≥ b9518). Vision capability is ground-truthed by the projector on disk, not the repo name, so multimodal models without a `-vl` hint (like Qwen 3.6) correctly show the **V** badge and accept attachments.
- **Fast Mode for GGUF (MTP speculative decoding) — including Mixture-of-Experts.** Fast Mode runs llama.cpp's `--spec-type draft-mtp` whether the MTP head is **embedded** in the weights (Qwen `…-MTP-GGUF`, self-speculative) or shipped as a **companion** `mtp-*.gguf` (Gemma 4). This is the Fast-Mode path that works for the **MoE** models MLX cannot accelerate. Validated on the M5 Max (Q4, draft acceptance shown in the server console): **Qwen 3.6 35B-A3B MoE → 112 t/s** (was 89/43; acc 0.85), **Gemma 4 26B-A4B MoE → 113 t/s** (acc 0.76), and the dense **Qwen 3.6 27B / Gemma 4 12B / 31B** all MTP-active — zero quality loss.
- **One-click Fast Mode download (bundle).** Picking a Fast-Mode-capable GGUF model offers **"Download with Fast Mode"**, which pulls the whole bundle in a single step — the model's recommended quant **plus** its MTP head and vision projector — with no second quant picker. The download row reads as the model (e.g. *"Qwen3.6-35B-A3B ⚡ Fast Mode (Q4_K_M, ~23 GB)"*), not the raw `…-MTP-GGUF` repo name. Identical flow for Qwen and Gemma.
- **QAT builds supported for Gemma 4 — both engines.** Quantization-aware-trained Gemma 4 works end-to-end on **GGUF** (`unsloth/gemma-4-*-qat-GGUF` — they ship the same `mtp-*.gguf` head, so selecting one downloads it automatically) and on **MLX** (`mlx-community/gemma-4-*-qat-*`, Fast Mode via the same `*-assistant-bf16` heads). QAT gives up nothing on 4-bit quality and runs measurably faster on Apple Silicon (**GGUF 26B-A4B qat Q4_K_XL → 137 t/s** vs 113 for the standard Q4_K_M).
- **Fast Mode for MLX (MTP speculative decoding) — Qwen 3.6 & Gemma 4, dense AND MoE.** Speculative decoding now runs on the MLX backend via the community's dedicated draft heads (Qwen `*-MTP-bf16`, Gemma `*-assistant-bf16`). Download a supported Qwen 3.6 or Gemma 4 model and the Fast Mode chip offers the matching optimizer; enabling it pulls a small draft head (~0.2–1.6 GB) and speeds up generation with **byte-identical output** (speculative decoding is mathematically exact). Dense speedups **×1.2–1.65** (Gemma 4 31B, Qwen 3.6 27B). **Mixture-of-Experts is now supported too** — Qwen 3.6 35B-A3B reaches **145 t/s** (4-bit) and Gemma 4 26B-A4B ~90 t/s, clean output. This was impossible until mlx-vlm 0.6.3 fixed the MoE+MTP corruption (see *Fixed*); Ka1zen **version-gates** MoE Fast Mode on mlx-vlm ≥ 0.6.3 (`SpeculativeDecoding.moeMTPSupported`, safe-default off), the MLX analog of the GGUF llama.cpp build gate — so an older install stays safe and it lights up automatically once the runtime is updated.
- **DiffusionGemma (Google's block-diffusion LM) runs via MLX.** `diffusiongemma-26B-A4B-it` — which generates by parallel denoising rather than left-to-right — loads and answers in-app through `mlx_vlm.server` (mlx-vlm 0.6.3's generate dispatcher auto-routes the diffusion path). Ka1zen suppresses per-token `logprobs` for diffusion models (they have none — the server would otherwise crash serializing a `None`), so generation is clean. Note: this is an **MLX-only** capability — the GGUF build of the same model cannot run here, because `llama-server` has no diffusion path (it needs the separate `llama-diffusion-cli`).
- **Per-quantization download picker for GGUF repos.** Picking a `…-GGUF` repo opens a sheet listing every quant (fetched live from the HF file tree) so you download only the one file you choose (Q4_K_M recommended; Q2/IQ1/IQ2 flagged "⚠︎ degrades"). The chosen quant plus its `mtp-*.gguf` and `mmproj-*.gguf` companions are fetched together.
- **GGUF browse sources.** The Browse filter adds the reference GGUF publishers — **ggml-org** (the llama.cpp team), **unsloth**, and **bartowski** — alongside the MLX sources (mlx-community, fcreait).
- **llama.cpp in the in-app prerequisite installer.** Settings → Runtime Health / "Update prerequisites" and "Restore validated versions" now install or upgrade llama.cpp via Homebrew too (non-fatal: an MLX-only user with no Homebrew is never blocked — the step is recorded as skipped). `install.sh` already pins the validated MLX stack and installs llama.cpp.
- **Pinned, reproducible engines.** All three inference engines are pinned so an end-user `pip install -U` or `brew upgrade` can't silently break the runtime: **mlx-lm 0.31.3**, **mlx-vlm 0.6.3** (unlocks MLX MoE Fast Mode + DiffusionGemma — see *Fixed*; 0.6.0 stays denylisted), and **llama.cpp b9585** as a managed binary under `~/Library/Application Support/Ka1zen/llama/`. `LlamaServerResolver` prefers that managed binary over any Homebrew `llama-server`, so the GGUF backend runs an exact build regardless of the moving Homebrew bottle (which lagged at 9430). b9585 clears both gates the Fast Mode path needs: Qwen MTP (≥ b9519) and **Gemma 4 MTP heads (≥ b9568)**, plus the `gemma4uv` 12B projector. `install.sh` downloads the pinned b9585 release directly (Homebrew fallback if the download fails).

### Changed

- **GGUF context window scales with unified memory, up to 128K.** llama.cpp pre-allocates the KV cache for the full context (unlike MLX, which grows it lazily), so the window is tiered by RAM — **≥64 GB → 131072 (128K)**, ≥32 GB → 64K, otherwise 32K — and pinned to a single conversation slot (`--parallel 1`) so the whole window goes to the active chat at minimal memory. Verified: Qwen 3.6 27B at 128K uses ~25 GB with decode speed unchanged.
- **GGUF vision robustness.** If a launch with `--mmproj` dies on a projector type the installed llama.cpp can't read, Ka1zen relaunches once text-only instead of failing the model outright — covering any unsupported projector, not just the known ones.
- **MLX KV cache runs at full f16 precision.** 8-bit KV-cache quantization stays off by default — on Apple Silicon's large unified memory full precision is the better default for quality, and the corruption it caused on ≤ 0.6.2 is now fixed upstream anyway (mlx-vlm 0.6.3, #1313 — see *Fixed*).

### Fixed

- **MLX Fast Mode on MoE no longer corrupts output.** Speculative decoding on a Mixture-of-Experts target (Qwen 3.6 35B-A3B, Gemma 4 26B-A4B) used to degenerate into gibberish at Apple-Silicon batch=1 — a French web-search prompt would loop into Chinese (upstream Blaizzy/mlx-vlm#1317, which Ka1zen had hard-disabled for MoE since 0.3.45). mlx-vlm 0.6.3 fixes it. Re-enablement was gated on proof, not the changelog: the fix was verified **clean across 7+ generations of the deterministic reproducer at temperatures 0.0–0.7, on both Qwen and Gemma MoE**, with the draft load confirmed in the server log, in an isolated environment — only then were the guards replaced with the `moeMTPSupported` version gate. (GGUF MTP was never affected — different engine.)
- **MLX KV-cache quantization no longer drops the prompt context.** On mlx-vlm ≤ 0.6.2, `--kv-bits` silently corrupted the prefilled prompt (the model answered as if the prompt were empty — in-context recall failed with no error). Fixed upstream in 0.6.3 (#1313); verified by recalling a number buried in an 8K-char context with `--kv-bits 8` active. Ka1zen still defaults KV to **f16** (the right call on ample-RAM Apple Silicon), but the quantized path is now safe.
- **DiffusionGemma no longer crashes on generation.** Requesting `logprobs` on a diffusion LM (which produces none) crashed `mlx_vlm.server` mid-stream (`float(None)`), so the model returned nothing. Ka1zen now omits `logprobs` for diffusion models — they generate cleanly.
- **GGUF download did nothing.** A server-side format guard rejected every `-GGUF` repo, so clicking Download / Install silently no-op'd. GGUF *chat* repos are now allowed through (image-format GGUF stays blocked).
- **GGUF Load button did nothing.** GGUF variant downloads registered their `ModelConfig` under a composite `repo/file.gguf` ID that never matched the bare repo ID the launcher looks up, so Load found nothing. GGUF now keys by the bare repo throughout, and the launcher tolerates older composite IDs.
- **GGUF rows mislabeled as MLX.** Installed GGUF models showed `mlx_lm`/`mlx_vlm` on their spec line and (for vision families) an MLX-vision badge; they now read **llama.cpp** and reflect the real backend.
- **A GGUF repo with no weight file is no longer offered as a broken MLX model** (e.g. an interrupted download or a stray projector fetch) — it's skipped instead of crashing `mlx_vlm.server` with "config.json not found".
- **Replies no longer cut off a few words early.** On long generations a UI-refresh race could commit the visible message slightly behind the model's full output, so the last words were dropped (and you'd have to hit Continue). The complete text is now always flushed before the message is saved.
- **Fewer spurious "Continue" prompts.** A finished reply ending in a list, heading, bold text, a quote, a percentage or a citation (e.g. "…[3]") was sometimes mistaken for a truncated one. The completeness check now recognizes these as complete.

### Thanks

This release stands on the work of the open-source community — the inference engines that run the models, and the publishers that make them runnable on a Mac. Most of what's new here was unlocked by their upstream work:

- **[mlx-vlm](https://github.com/Blaizzy/mlx-vlm)** (Prince Canuma and contributors) — the MLX vision/LLM server Ka1zen drives. 0.6.3 fixed the MoE + MTP corruption (#1317) and the KV-cache prompt bug (#1313), and its generate dispatcher is what makes DiffusionGemma run. Fast Mode on Apple Silicon simply wouldn't exist without it.
- **[llama.cpp](https://github.com/ggml-org/llama.cpp)** (Georgi Gerganov and the ggml-org team) — the GGUF backend, its Metal kernels, and the `--spec-type draft-mtp` speculative path that accelerates MoE models day-one.
- **[mlx-community](https://huggingface.co/mlx-community)** — the MLX model conversions and the dedicated MTP draft heads (`*-MTP-bf16`, `*-assistant-bf16`) that every MLX Fast Mode pairing depends on, plus the DiffusionGemma and QAT MLX builds.
- **[unsloth](https://huggingface.co/unsloth)** — the GGUF quantizations, the bundled / self-speculative `mtp-*.gguf` heads that power GGUF Fast Mode, and the QAT GGUF builds.
